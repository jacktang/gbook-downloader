...
find the answer for the question: If one book is composed with 500 pages and only 20 pages 
is public, how many proxies/requests should send in order to get one completed book?
...
* ProxyManager invokes ProxyProvider one by one, and state the common proxy tests. If the proxy instance passed, it will be cached, meanwhile, PageCollector adopts the proxy candidate.
